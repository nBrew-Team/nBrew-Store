...
void Logging::createLogFile(void) {
	if ((access("sdmc:/3ds/nBrew-Store/Log.log", F_OK) != 0)) {
		FILE* logFile = fopen(("sdmc:/3ds/nBrew-Store/Log.log"), "w");
		fclose(logFile);
	}
}
...
void Logging::writeToLog(std::string debugText) {
	if (config->logging()) {
		...
		logFile.open(("sdmc:/3ds/nBrew-Store/Log.log"), std::ofstream::app);
